...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("verify-otp-btn").addEventListener("click", async () => {
        ...
        try {
            const response = await fetch("https://localhost:8443/authenticator/verify_otp/", {
                method: "POST",
    ...
    document.getElementById("back-to-qr-btn").addEventListener("click", () => {
        window.location.href = `https://localhost:8443/authenticator/qr/?email=${encodeURIComponent(email)}}`;
    });
});
